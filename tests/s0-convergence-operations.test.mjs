...
const cmd = (client, request) =>
  client.rpc('execute_command', {
    p_request: { command_id: randomUUID(), ...request }
  });
const read = (client, readType, params = {}) =>
  client.rpc('execute_operations_read', {
    p_request: { read_type: readType, ...params }
  });
const mode = async (fn) =>
  (
    await service
...
before(async () => {
  ...
  [lenny, dave, tanya, lucy, ben, rick, james] = await Promise.all(
    names.map((n) => signInAs(email(n)))
  );
  for (const [k, legacy] of [
    ['lenny', 'PERSON-lenny-dev'],
    ['dave', 'PERSON-dave-hopwood'],
    ['tanya', 'PERSON-tanya'],
    ['lucy', 'PERSON-lucy'],
    ['rick', 'PERSON-rick'],
    ['ben', 'PERSON-ben'],
  ...
  if (fn01.mode === 'Disabled')
    assert.ifError((await setMode(lenny, 'FN-01', fn01, 'Automated')).error);
  const sold = await tanya.rpc('submit_presale', {
    p_command_id: randomUUID(),
    p_payload: {
      customer: {
        first_name: 'Cora',
        last_name: 'Convergence',
        address_line1: '5 Synthetic Way',
        address_line2: null,
        town: 'Exeter',
        postcode: 'EX5 5EE',
        phone: '07000 000005',
        email: `cora-${Date.now()}@example.com`
      },
      sale: {
        salesperson_id: ids.rick,
        lead_source: 'Referral',
        quote_reference: `Q-CONV-${Date.now()}`,
        finance_route: 'Standard',
        agreed_price_pence: 700000
      },
      scope: {
        roof_required: true,
      ...
      computed: {
        system_kwp: 4,
        net_panels: 10,
        computed_total_pence: 700000,
        price_breakdown: [{ key: 'total', label: 'Total', pence: 700000 }]
      }
    }
  });
...
after(async () => {
  ...
  const rows =
    (await service.from('release_modes').select('*').neq('mode', 'Disabled'))
      .data ?? [];
  await service
    .from('release_modes')
...
describe('release control', () => {
  test('only Admin / Manager switch; Office and Director are refused', async () => {
    ...
    for (const who of [tanya, ben])
      assert.equal(
        (await setMode(who, 'FN-18', fn18, 'Manual')).error?.message,
        'R1A_ROLE_DENIED'
      );
    assert.equal(
      (await read(tanya, 'RELEASE_CONTROL')).error?.message,
      'R1A_ROLE_DENIED'
    );
  ...
  test('planned mode only, reason required, version checked, one audit event', async () => {
    ...
    assert.equal(
      (await setMode(dave, 'FN-18', fn18, 'Automated')).error?.message,
      'RELEASE_MODE_NOT_PLANNED'
    );
    assert.equal(
      (await setMode(dave, 'FN-18', fn18, 'Manual', 'x')).error?.message,
    ...
    const perCommand = (
      await service
        .from('audit_events')
        .select('id')
        .eq('command_id', audit.command_id)
    ).data;
    assert.equal(
      perCommand.length,
      1,
      'exactly one audit event per release change'
    );
  ...
  test('dependencies hold both ways; direct writes are refused', async () => {
    ...
    assert.equal(
      ...
      'RELEASE_DEPENDANT_ENABLED',
      'FN-18 still needs FN-01'
    );
    const fn18 = await mode('FN-18');
    assert.ifError((await setMode(lenny, 'FN-18', fn18, 'Disabled')).error);
    assert.ifError(
      (await setMode(lenny, 'FN-01', await mode('FN-01'), 'Disabled')).error
    );
    const fn19 = await mode('FN-19');
    assert.equal(
      (await setMode(lenny, 'FN-19', fn19, 'Manual')).error?.message,
      'RELEASE_DEPENDENCY_DISABLED'
    );
    assert.ifError(
      (await setMode(lenny, 'FN-01', await mode('FN-01'), 'Automated')).error
    );
    const direct = await lenny
      .from('release_modes')
      .update({ mode: 'Automated' })
      .eq('function_id', 'FN-02');
  ...
  test('readiness never counts missing evidence as a pass', async () => {
    ...
    const items = Object.fromEntries(
      r.data.data.items.map((i) => [i.key, i.state])
    );
    ...
    assert.equal(
      (await read(rick, 'RELEASE_READINESS')).error?.message,
      'R1A_ROLE_DENIED'
    );
  });
});
...
describe('staff access', () => {
  test('Manager adds a person; only an Admin gives Admin', async () => {
    const created = await cmd(dave, {
      command_type: 'STAFF_CREATE',
      payload: {
        display_name: 'Sam Convergence',
        email: `sam.conv.${Date.now()}@example.com`,
        roles: ['Office']
      }
    });
    ...
    const row = (
      await service.from('people').select('*').eq('id', personId).single()
    ).data;
    assert.equal(
      (
        await cmd(dave, {
  ...
  test('no self lock-out, never the last Admin; deactivation takes effect at once', async () => {
    const me = (
      await service.from('people').select('*').eq('id', ids.lenny).single()
    ).data;
    assert.equal(
      (
        await cmd(lenny, {
          ...
          payload: { person_id: ids.lenny, active: false, reason: 'test' }
        })
      ).error?.message,
      'STAFF_SELF_LOCKOUT'
    );
    ...
    assert.equal(
      (
        await cmd(lenny, {
          command_type: 'STAFF_ROLE_SET',
          expected_version: adminRole.version,
          payload: {
            person_id: ids.lenny,
            role_code: 'Admin',
            active: false,
    ...
    const lucyRow = (
      await service.from('people').select('*').eq('id', ids.lucy).single()
    ).data;
    assert.ifError(
      (
        await cmd(lenny, {
          command_type: 'STAFF_SET_ACTIVE',
          expected_version: lucyRow.version,
          payload: { person_id: ids.lucy, active: false, reason: 'on leave' }
        })
      ).error
    );
    try {
      assert.equal(
        (await read(lucy, 'TASKS', { scope: 'my' })).error?.message,
        'R1A_INACTIVE_ACTOR'
      );
    } finally {
      const again = (
      ...
      assert.ifError(
        (
          await cmd(lenny, {
            ...
            payload: { person_id: ids.lucy, active: true, reason: 'back' }
          })
        ).error
      );
    }
...
describe('task reassignment', () => {
  test('eligible people only, version checked, owner changes, history kept', async () => {
    const cands = await read(tanya, 'TASK_REASSIGN_CANDIDATES', {
      task_id: pre01.id
    });
    ...
    assert.equal(
      (
        await cmd(tanya, {
          ...
          task_id: pre03.id,
          expected_version: pre03.version,
          payload: { owner_id: ids.rosie, reason: 'cover' }
        })
      ).error?.message,
      'TASK_OWNER_NOT_ELIGIBLE',
      'PRE03 is Admin/Manager/Director only'
    );
    assert.equal(
      (
        await cmd(rick, {
          command_type: 'TASK_REASSIGN',
          task_id: pre01.id,
          expected_version: pre01.version,
          payload: { owner_id: ids.rosie, reason: 'cover' }
        })
      ).error?.message,
      'R1A_ROLE_DENIED'
    );
    const moved = await cmd(tanya, {
      command_type: 'TASK_REASSIGN',
      task_id: pre01.id,
      expected_version: pre01.version,
      payload: {
    ...
    const after = (
      await service.from('tasks').select('*').eq('id', pre01.id).single()
    ).data;
    ...
    assert.equal(
      (
        await cmd(tanya, {
          ...
          task_id: pre01.id,
          expected_version: pre01.version,
          payload: { owner_id: ids.tanya, reason: 'again' }
        })
      ).error?.message,
      'R1A_STALE_VERSION'
    );
    const ev = (
      await service.from('task_events').select('*').eq('task_id', pre01.id)
    ).data;
...
describe('issues queue', () => {
  test('office sees open issues across jobs with actions; other roles are refused', async () => {
    const j = (
      await service.from('jobs').select('version').eq('id', job).single()
    ).data;
    const raised = await cmd(tanya, {
      ...
      payload: {
        ...
        title: 'Gutter',
        description: 'Gutter bracket loose',
        customer_impact: 'yes'
      }
    });
    ...
    const issue = q.data.data.issues.find(
      (i) => i.id === raised.data.result.issue_id
    );
    ...
    for (const who of [rick, james])
      assert.equal(
        (await read(who, 'ISSUES')).error?.message,
        'R1A_ROLE_DENIED'
      );
    assert.equal(
      (await read(tanya, 'ISSUES', { status: 'nope' })).error?.message,
      'R1A_INVALID_FIELDS'
    );
  });
});
  ...
  test('authorized results only, never a storage path', async () => {
    ...
    const reg = await tanya.rpc('evidence_upload_begin', {
      p_request: {
        ...
        context_id: job,
        category: 'Contract',
        filename: 'conv-contract.pdf',
        mime_type: 'application/pdf',
        size_bytes: PDF.length
      }
    });
    assert.ifError(reg.error);
    const ticket = await tanya.storage
      .from('evidence')
      .createSignedUploadUrl(reg.data.storage_path);
    assert.ifError(
      (
        await tanya.storage
          .from('evidence')
          .uploadToSignedUrl(reg.data.storage_path, ticket.data.token, PDF, {
    ...
    const found = await tanya.rpc('search_evidence', {
      p_request: { q: 'conv-contract' }
    });
    assert.ifError(found.error);
    assert.ok(
      found.data.files.some(
        (f) => f.id === reg.data.evidence_id && f.postcode === 'EX5 5EE'
      )
    );
    assert.ok(found.data.files.every((f) => !('storage_path' in f)));
    const byPostcode = await tanya.rpc('search_evidence', {
      p_request: { q: 'EX5 5EE', category: 'Contract' }
    });
    assert.ok(byPostcode.data.files.some((f) => f.id === reg.data.evidence_id));
    const installer = await james.rpc('search_evidence', {
      p_request: { q: 'conv-contract' }
    });
    assert.ifError(installer.error);
    assert.equal(
      installer.data.files.length,
      0,
      'an installer never sees contracts'
    );
    assert.ok((await anon.rpc('search_evidence', { p_request: {} })).error);
    assert.equal(
      (await tanya.rpc('search_evidence', { p_request: { bogus: 1 } })).error
        ?.message,
      'R1A_INVALID_FIELDS'
    );
  });
