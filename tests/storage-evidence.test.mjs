...
const sale = (salespersonId, n) => ({
  customer: {
    ...
    postcode: 'EX1 1AA',
    phone: '07000 000000',
    email: `evi${n}@example.com`
  },
  sale: {
    salesperson_id: salespersonId,
    lead_source: 'Referral',
    quote_reference: `Q-EV-${n}`,
  ...
  computed: {
    system_kwp: 5.1,
    net_panels: 10,
    computed_total_pence: 1250000,
    price_breakdown: [{ key: 'total', label: 'Total', pence: 1250000 }]
  }
});

const begin = (client, contextId, over = {}) =>
  client.rpc('evidence_upload_begin', {
    p_request: {
...
async function uploadFor(client, taskId, over = {}) {
  ...
  const sent = await bucket(client).uploadToSignedUrl(
    reg.data.storage_path,
    ticket.data.token,
    PDF,
...
before(async () => {
  ...
  for (const name of ['tanya', 'lucy', 'john', 'rick'])
    await ensureLogin(email(name));
  [tanya, lucy, john, rick] = await Promise.all(
    ['tanya', 'lucy', 'john', 'rick'].map((n) => signInAs(email(n)))
  );
  ...
  for (const n of [1, 2]) {
    const sold = await tanya.rpc('submit_presale', {
      ...
      p_payload: sale(rickRow.id, `${n}-${Date.now()}`)
    });
    assert.ifError(sold.error);
    if (n === 1) jobA = sold.data.job_id;
    else jobB = sold.data.job_id;
  }
  const tasks = await service
    .from('tasks')
    .select('*')
    .in('job_id', [jobA, jobB])
    .eq('template_code', 'PRE02');
...
describe('upload', { skip }, () => {
  test('registered upload through a signed URL; storage is the witness', async () => {
    ...
    assert.ok(
      uploaded.storage_path.startsWith(`${jobA}/${uploaded.evidence_id}/`)
    );
    const { data: row } = await service
      .from('evidence')
      .select('*')
      .eq('id', uploaded.evidence_id)
      .single();
  ...
  test('a retry with the same upload id is the same evidence; the stored file cannot be overwritten', async () => {
    ...
    const first = await begin(tanya, pre02A.id, {
      upload_id: uploadId,
      filename: 'retry.pdf'
    });
    const again = await begin(tanya, pre02A.id, {
      upload_id: uploadId,
      filename: 'retry.pdf'
    });
    assert.equal(again.data.evidence_id, first.data.evidence_id);
    assert.equal(again.data.replayed, true);
    const ticket = await bucket(tanya).createSignedUploadUrl(
      first.data.storage_path
    );
    assert.ifError(ticket.error);
    assert.ifError(
      (
        await bucket(tanya).uploadToSignedUrl(
          first.data.storage_path,
          ticket.data.token,
          PDF,
          { contentType: 'application/pdf' }
    ...
    const second = await bucket(tanya).createSignedUploadUrl(
      first.data.storage_path
    );
    assert.ok(
      second.error,
      'no second upload URL for a path that already holds a file'
    );
    const done = await tanya.rpc('evidence_upload_complete', {
      p_evidence_id: first.data.evidence_id
    });
    assert.equal(done.data.upload_status, 'Uploaded');
    assert.equal(
      (
        await begin(tanya, pre02A.id, {
          upload_id: uploadId,
          filename: 'retry.pdf'
        })
      ).data.upload_status,
  ...
  test('storage itself refuses the wrong type and an oversized file', async () => {
    ...
    let ticket = await bucket(tanya).createSignedUploadUrl(
      html.data.storage_path
    );
    const asHtml = await bucket(tanya).uploadToSignedUrl(
      html.data.storage_path,
      ticket.data.token,
    ...
    assert.equal(
      (
        await tanya.rpc('evidence_upload_complete', {
          p_evidence_id: html.data.evidence_id
        })
      ).error?.message,
      'R1A_UPLOAD_MISSING'
    );

    const big = await begin(tanya, pre02A.id, {
      filename: 'big.pdf',
      size_bytes: 1000
    }); // lies about the size
    ticket = await bucket(tanya).createSignedUploadUrl(big.data.storage_path);
    const tooBig = await bucket(tanya).uploadToSignedUrl(
      big.data.storage_path,
      ticket.data.token,
      Buffer.alloc(25 * 1024 * 1024 + 1024, 1),
      { contentType: 'application/pdf' }
    );
    ...
    assert.equal(
      (
        await tanya.rpc('evidence_upload_complete', {
          p_evidence_id: big.data.evidence_id
        })
      ).error?.message,
      'R1A_UPLOAD_MISSING'
    );
  ...
  test('nobody writes to a path that is not their own pending registration', async () => {
    ...
    for (const [who, client, path] of [
      ...
      [
        'uploader, sibling of a registered path',
        tanya,
        reg.data.storage_path.replace('mine.pdf', 'other.pdf')
      ],
      ...
      assert.ok(
        (
          await bucket(client).upload(path, PDF, {
            contentType: 'application/pdf'
          })
        ).error,
        `${who}: direct upload`
      ...
      assert.ok(
        (await bucket(client).createSignedUploadUrl(path)).error,
        `${who}: signed upload url`
      );
    }
    // an uploaded file can be neither replaced, moved nor removed by staff
    assert.ok(
      (
        await bucket(tanya).upload(uploaded.storage_path, PDF, {
          contentType: 'application/pdf',
          upsert: true
        })
      ).error
    ...
    assert.ok(
      (await bucket(tanya).move(uploaded.storage_path, `${jobB}/stolen.pdf`))
        .error
    );
    await bucket(tanya).remove([uploaded.storage_path]);
    assert.ok(
      (await service.storage.from('evidence').download(uploaded.storage_path))
        .data,
      'remove() by staff deleted the file'
    );
  });
});
...
describe('open / download', { skip }, () => {
  test('authorized staff get a short-lived URL that serves the file', async () => {
    for (const client of [tanya, lucy, rick]) {
      // owner, other office (job.read.all), the salesperson
      const open = await client.rpc('evidence_open', {
        p_evidence_id: uploaded.evidence_id
      });
      assert.ifError(open.error);
      const signed = await bucket(client).createSignedUrl(
  ...
  test('knowing the path gains an unauthorized person nothing', async () => {
    assert.equal(
      (await john.rpc('evidence_open', { p_evidence_id: uploaded.evidence_id }))
        .error?.message,
      'EVIDENCE_ACCESS_DENIED'
    );
    assert.ok(
      (await anon.rpc('evidence_open', { p_evidence_id: uploaded.evidence_id }))
        .error
    );
    for (const [who, client] of [
      ...
      assert.ok(
        (await bucket(client).download(uploaded.storage_path)).error,
        `${who}: download`
      );
      assert.ok(
        (await bucket(client).createSignedUrl(uploaded.storage_path, 60)).error,
        `${who}: signed url`
      );
      assert.deepEqual(
        (await bucket(client).list(jobA)).data ?? [],
        [],
        `${who}: list`
      );
      assert.deepEqual(
        (await client.from('evidence').select('id').eq('job_id', jobA)).data ??
          [],
        [],
        `${who}: table`
      );
    ...
    const pub = await fetch(
      `${API_URL}/storage/v1/object/public/evidence/${uploaded.storage_path}`
    );
    assert.ok(pub.status >= 400);
    const bare = await fetch(
      `${API_URL}/storage/v1/object/evidence/${uploaded.storage_path}`
    );
    assert.ok(bare.status >= 400);
    // the list never carries a path
    const list = await tanya.rpc('list_evidence', {
      p_request: { job_id: jobA }
    });
    assert.ok(list.data.evidence.length >= 1);
    assert.ok(
      list.data.evidence.every(
        (e) => !('storage_path' in e) && e.job_id === jobA
      )
    );
    assert.deepEqual(
      (await john.rpc('list_evidence', { p_request: { job_id: jobA } })).data
        .evidence,
      []
    );
  });
  ...
  test('an inactive person opens nothing, through the database or Storage', async () => {
    ...
    try {
      ...
      assert.ok(
        (await bucket(lucy).createSignedUrl(uploaded.storage_path, 60)).error
      );
...
describe('commands', { skip }, () => {
  const complete = (client, task, path) =>
  ...
  test('a file of job A cannot complete a task of job B; a dropped-in file is never adopted', async () => {
    assert.equal(
      (await complete(tanya, pre02B, uploaded.storage_path)).error?.message,
    ...
    assert.equal(
      (await complete(tanya, pre02B, stray)).error?.message,
      'R1A_UPLOAD_INVALID'
    );
    assert.ok(
      (await bucket(tanya).download(stray)).error,
      'a file without metadata is readable'
    );
  ...
  test('a registered file that never arrived completes nothing; once stored, the command confirms it', async () => {
    ...
    assert.equal(
      (await complete(tanya, pre02B, reg.data.storage_path)).error?.message,
      'R1A_UPLOAD_MISSING'
    );
    assert.equal(
      (
    ...
    const job = await service
      .from('jobs')
      .select('contract_status, contract_evidence_id')
      .eq('id', jobB)
      .single();
    ...
    const actions = await service
      ...
      .eq('entity_id', reg.data.evidence_id);
    assert.deepEqual(actions.data.map((a) => a.action).sort(), [
      'Attach',
      'Register',
      'TaskLink',
      'Upload'
...
describe('consistency', { skip }, () => {
  test('metadata whose file is gone is reported once, and shows up for admins', async () => {
    const gone = await uploadFor(tanya, pre02A.id, {
      filename: 'will-vanish.pdf'
    });
    assert.ifError(
      (await service.storage.from('evidence').remove([gone.storage_path])).error
    );
    assert.ifError(
      (await tanya.rpc('evidence_open', { p_evidence_id: gone.evidence_id }))
        .error
    );
    assert.ok(
      (await bucket(tanya).createSignedUrl(gone.storage_path, 60)).error
    );
    for (let i = 0; i < 2; i += 1)
      assert.equal(
        (
          await tanya.rpc('evidence_report_missing', {
            p_evidence_id: gone.evidence_id
          })
        ).data.file_present,
        false
      );
    const events = await service
      .from('audit_events')
      .select('id')
      .eq('entity_id', gone.evidence_id)
      .eq('action', 'FileMissing');
    assert.equal(events.data.length, 1);
    assert.equal(
      (await tanya.rpc('evidence_consistency')).error?.message,
      'R1A_ROLE_DENIED'
    );
    await ensureLogin(email('lenny'));
    const report = await (
      await signInAs(email('lenny'))
    ).rpc('evidence_consistency');
    assert.ifError(report.error);
    assert.ok(
      report.data.file_missing.some((m) => m.evidence_id === gone.evidence_id)
    );
    assert.ok(
      report.data.object_without_metadata.length >= 1,
      'the dropped-in file'
    );
  });
});
