...
const item = async (batch, taskId) =>
  one(
    `select * from public.command_batch_items where batch_id=$1 and task_id=$2`,
    [batch, taskId]
  );
const batchRow = async (batch) =>
  one(`select * from public.command_batches where id=$1`, [batch]);

/** Submit a batch and drive it to completion the way the app does. */
async function runBatch(who, payload, limit = 25) {
  const r = await cmd(who, {
    command_id: id(),
  ...
  for (;;) {
    const chunk = (
      await db.query(`select public.run_batch_chunk($1::uuid, $2::int) r`, [
        batch,
        limit
      ])
    ).rows[0].r;
    if (
      chunk.ran === 0 ||
      (chunk.progress.pending === 0 && chunk.progress.retrying === 0)
    )
      break;
  ...
  return {
    ...r,
    batch,
    progress: (await db.query(`select app.batch_progress($1::uuid) p`, [batch]))
      .rows[0].p
  };
}
...
const soldB = await sell('tanya', {
  customer: { first_name: 'Bob', last_name: 'Jones', postcode: 'LS2 2BB' }
});
...
// ---------------------------------------------------------------------------
{
  ...
  assert.ok(
    !by.PRE03.blocking.some((b) => b.code === 'ownership'),
    JSON.stringify(by.PRE03.blocking)
  );
...
// ---------------------------------------------------------------------------
{
  const r = await runBatch('tanya', {
    operation: 'TASK_BATCH_COMPLETE',
    completion_note: 'Checked with provider',
    task_ids: [a01.id, a03.id, aNote.id]
  });
...
// ---------------------------------------------------------------------------
{
  ...
  assert.ok(
    bypassed.includes('deposit_bank_confirmed'),
  ...
  const stage = await one(
    `select * from public.invoice_stages where job_id=$1 and stage='Deposit'`,
    [jobA]
  );
  assert.equal(
    stage.sent_at,
    null,
    'override must not stamp the deposit invoice'
  );
  assert.equal(stage.invoice_number, null);
  const checks = await all(
    `select * from public.manual_bank_checks where job_id=$1`,
    [jobA]
  );
  ...
  const ev = await one(
    `select * from public.task_events where task_id=$1 and action='OverrideComplete'`,
    [a03.id]
  );
  ...
  const au = await one(
    `select * from public.audit_events where entity_id=$1 and action='OverrideComplete'`,
    [a03.id]
  );
...
// ---------------------------------------------------------------------------
{
  ...
  assert.equal(
    job.workflow_stage,
    'Prebooking',
    'override must never promote a job'
  );
  const debt = (
    await db.query(`select app.job_override_debt($1::uuid) d`, [jobA])
  ).rows[0].d;
  assert.equal(debt.length, 2);
  const codes = debt.map((d) => d.template_code).sort();
  assert.deepEqual(codes, ['PRE01', 'PRE03']);
  assert.ok(
    debt[0].unrecorded.length > 0,
    'the unrecorded facts must be listed'
  );
  assert.ok(debt.every((d) => d.override_by === 'Tanya'));
  console.log(
    'override debt:',
    debt.map((d) => `${d.template_code}(${d.unrecorded.length})`).join(' ')
  );

  // The readiness gate still reports the real requirement as unmet.
  const readiness = (
    await db.query(`select app.evaluate_ready_to_book($1::uuid) r`, [jobA])
  ).rows[0].r;
...
// ---------------------------------------------------------------------------
{
  ...
  const payload = {
    operation: 'TASK_BATCH_OVERRIDE_COMPLETE',
    override_reason: 'Duplicate requirement',
    task_ids: [(await noteOnlyTask(jobB)).id]
  };
  const first = await cmd('tanya', {
  ...
  assert.equal(
    (await all(`select * from public.command_batches`)).filter(
      (b) => b.command_id === commandId
    ).length,
    1
  );
  ...
  const p1 = (await db.query(`select app.batch_progress($1::uuid) p`, [batch]))
    .rows[0].p;
  assert.equal(p1.succeeded, 1);
  const rerun = (
    await db.query(`select public.run_batch_chunk($1::uuid, 25) r`, [batch])
  ).rows[0].r;
  assert.equal(rerun.ran, 0);
  // Exactly one commands-ledger row, one task_event.
  const it = await one(
    `select * from public.command_batch_items where batch_id=$1`,
    [batch]
  );
  assert.equal(
    (
      await all(`select * from public.commands where command_id=$1`, [
        it.command_id
      ])
    ).length,
    1
...
// ---------------------------------------------------------------------------
{
  const r = await runBatch('tanya', {
    operation: 'TASK_BATCH_OVERRIDE_COMPLETE',
    override_reason: 'Historical admin cleanup',
...
// ---------------------------------------------------------------------------
{
  ...
  const r = await cmd('tanya', {
    ...
    payload: {
      operation: 'TASK_BATCH_OVERRIDE_COMPLETE',
      override_reason: 'Manager instruction',
      task_ids: [b02.id]
    }
  });
  const batch = r.result.batch_id;
  ...
  ok(
    await cmd('tanya', {
      ...
      task_id: b02.id,
      expected_version: b02.version,
      payload: {
        completion_note: 'sent to customer',
        contract_id: 'SIG-9',
  ...
  it = await one(`select * from public.command_batch_items where batch_id=$1`, [
    batch
  ]);
...
// ---------------------------------------------------------------------------
{
  ...
  assert.equal(
    (await one(`select * from public.tasks where id=$1`, [b03.id])).status,
    'Open'
  );
  ...
  const r2 = await runBatch('dan', {
    operation: 'TASK_BATCH_OVERRIDE_COMPLETE',
    override_reason: 'Confirmed externally',
    task_ids: [b03.id]
  });
...
// ---------------------------------------------------------------------------
{
  const r = await runBatch('tanya', {
    ...
    reopen_reason: 'Reverting the cleanup',
    task_ids: [a01.id, a03.id, aNote.id]
  });
  ...
  const actions = (
    await all(
      `select action from public.task_events where task_id=$1 order by occurred_at, id`,
      [a01.id]
    )
  ).map((e) => e.action);
  ...
  assert.equal(
    (await one(`select * from public.tasks where id=$1`, [aNote.id]))
      .completion_note,
    'Checked with provider'
  );
  // The debt clears with the override.
  assert.equal(
    (await db.query(`select app.job_override_debt($1::uuid) d`, [jobA])).rows[0]
      .d.length,
    0
  );
...
// ---------------------------------------------------------------------------
{
  const r = await runBatch('ben', {
    ...
    owner_id: people.dan,
    reason: 'Cover while away',
    task_ids: [a01.id, aNote.id]
  });
  assert.equal(r.progress.succeeded, 2, JSON.stringify(r.progress));
  assert.equal(
    (await one(`select * from public.tasks where id=$1`, [a01.id])).owner_id,
    people.dan
  );
  const ev = await one(
    `select * from public.task_events where task_id=$1 and action='Reassign'`,
    [a01.id]
  );
...
// ---------------------------------------------------------------------------
{
  ...
  assert.ok(
    pf.data.items.every((i) => i.template_code.startsWith('PRE')),
    'code_prefix must filter'
  );
...
// ---------------------------------------------------------------------------
{
  ...
  const single = await cmd('tanya', {
    ...
    command_type: 'TASK_COMPLETE',
    task_id: bTask.id,
    expected_version: bTask.version,
    payload: {
      completion_note: 'x',
  ...
  assert.ok(
    pf.data.items[0].blocking.some(
      (b) => b.code === 'live_job' && b.kind === 'hard'
    )
  );

  const r = await runBatch('tanya', {
    operation: 'TASK_BATCH_OVERRIDE_COMPLETE',
    override_reason: 'DEV/test',
    task_ids: [bTask.id]
  });
  assert.equal(r.progress.succeeded, 0);
  assert.equal(
    (await one(`select * from public.tasks where id=$1`, [bTask.id])).status,
    'Open'
  );
  await db.query(
    `update public.jobs set record_class='Live', source_system=null, archived_at=null where id=$1`,
    [jobB]
  );
...
// ---------------------------------------------------------------------------
{
  ...
  if (callTask) {
    const pf = await readOps('tanya', {
      read_type: 'BATCH_PREFLIGHT',
      operation: 'TASK_BATCH_OVERRIDE_COMPLETE',
      task_ids: [callTask.id]
    });
    assert.equal(pf.data.items[0].outcome, 'not_actionable');
    assert.ok(
      pf.data.items[0].blocking.some(
        (b) => b.code === 'not_completed_elsewhere'
      )
    );
...
// ---------------------------------------------------------------------------
{
  ...
  assert.equal(
    ...
    12,
    'an override can take PRE02 as well'
  );

  // One bounded chunk at a time, as the browser drives it.
  await as('tanya');
  const c1 = (
    await db.query(`select public.run_batch_chunk($1::uuid, 5) r`, [batch])
  ).rows[0].r;
  ...
  const p = (await db.query(`select app.batch_progress($1::uuid) p`, [batch]))
    .rows[0].p;
...

{
  ...
  const s = await sell('tanya', {
    customer: { first_name: 'Stall', last_name: 'Test', postcode: 'LS4 4DD' }
  });
...
{
  const s = await sell('tanya', {
    customer: { first_name: 'Owned', last_name: 'Test', postcode: 'LS5 5EE' }
  });
  const t = await noteOnlyTask(s.job_id);
  const r = await cmd('tanya', {
    command_id: id(),
  ...
  const denied = await readOps('hannah', {
    read_type: 'BATCH_DETAIL',
    batch_id: batch
  });
  assert.match(denied.error ?? '', /BATCH_ACCESS_DENIED|ROLE_DENIED/);
  const allowed = await readOps('ben', {
    read_type: 'BATCH_DETAIL',
    batch_id: batch
  });
...
// ---------------------------------------------------------------------------
{
  const cls = async (state, msg) =>
    (await db.query(`select app.batch_error_class($1,$2) c`, [state, msg]))
      .rows[0].c;
...
// ---------------------------------------------------------------------------
{
  ...
  const detail = await readOps('tanya', {
    read_type: 'BATCH_DETAIL',
    batch_id: list.data.batches[0].batch_id
  });
  assert.ok(detail.ok, JSON.stringify(detail));
  assert.ok(Array.isArray(detail.data.items));
  assert.ok(
    detail.data.items.every((i) => 'template_code' in i && 'status' in i)
  );

  // An admin can see everyone's.
  const everyone = await readOps('ben', {
    read_type: 'BATCHES',
    all: true,
    limit: 50
  });
  ...
  console.log(
    'processing centre:',
    everyone.data.batches.length,
    'operations visible to an admin'
  );
}
...
{
  const s = await sell('tanya', {
    customer: { first_name: 'Visible', last_name: 'Test', postcode: 'LS6 6FF' }
  });
  const t = await task(s.job_id, 'PRE01');
  const r = await runBatch('tanya', {
    operation: 'TASK_BATCH_OVERRIDE_COMPLETE',
    override_reason: 'Invoice raised outside the system',
    task_ids: [t.id]
  });
  assert.equal(r.progress.succeeded, 1);
  ...
  assert.ok(
    row.override_unrecorded.length > 0,
    'the list says what was not recorded'
  );
  ...
  const debt = await readOps('tanya', {
    read_type: 'JOB_OVERRIDE_DEBT',
    job_id: s.job_id
  });
  ...
  console.log(
    'override visible in lists and on the job:',
