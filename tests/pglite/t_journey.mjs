...
assert.equal(r.workflow_stage, 'BookingInProgress');
...
console.log(
  ...
  r.status,
  'tasks',
  JSON.stringify(r.booking_tasks.created)
);
    ...
    await (async () => {
      ...
      return (
        await db.query(`select public.execute_read($1::jsonb) r`, [
          { read_type: 'ACTION_AVAILABILITY', job_id: job }
        ])
      ).rows[0].r;
    })(),
    'avail'
...
assert.deepEqual(r.tasks_created, ['BKG04', 'BKG05']);

// Work done: actual ends recorded; schedules raise INS01 per package
await db.query(
  `update public.work_packages set actual_start=planned_start, actual_end=planned_end where job_id=$1`,
  [job]
);
...
for (const t of await all(
  `select * from public.tasks where job_id=$1 and template_code='INS01'`,
  [job]
)) {
  ok(
    await cmd('tanya', {
...
for (const wp of await all(
  `select * from public.work_packages where job_id=$1 and commissioning_required`,
  [job]
)) {
  ok(
    await cmd('tanya', {
      ...
      payload: {
        evidence_path: `${job}/${wp.trade}-cert.pdf`,
        reference: `CERT-${wp.trade}`
      }
    }),
    'commissioning'
  );
...
assert.ok(
  await one(
    `select 1 x from public.invoice_stages where job_id=$1 and stage='Balance'`,
    [job]
  )
);
...
const open = await all(
  `select id, template_code, version from public.tasks where job_id=$1 and task_group='Cancellation' and status in ('Open','Blocked')`,
  [second.job]
);
...
r = ok(
  await cmd('tanya', {
    ...
    expected_version: j.version,
    payload: {
      reason: 'All obligations tracked',
      tracked_obligations: open.map((t) => ({
        task_id: t.id,
        reference: 'tracked',
...
r = ok(
  await cmd('tanya', {
    ...
    payload: {
      ...
      new_date: '2026-11-02',
      commitment_review: 'ok',
      finance_review: 'ok',
      evidence_reference: 'EMAIL-1',
      risk_review: 'Deposit held; contract re-issued'
    }
  }),
  'reinstate'
);
...
console.log(
  'after reinstatement BOOKING_GATES ->',
  r.error ?? r.result.status,
  (r.result?.readiness?.gates ?? [])
    .filter((g) => !g.pass)
    .map((g) => g.name)
    .join(',')
);
...
console.log(
  (
    await one(
      `select public.describe_command_error('R1A_STALE_VERSION', 'cmd-x') r`
    )
  ).r.message
);
const c = await one(
  `select (select count(*) from public.audit_events)::int a, (select count(*) from public.commit_journal)::int j`
);
