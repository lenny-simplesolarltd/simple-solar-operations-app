...
const C = (type, payload, extra = {}) => ({ command_id: id(), command_type: type, ...extra, payload });
const commRow = (c) => one(`select * from public.communications where id=$1`, [c]);
...
const snapshot = async () => {
  const out = {};
  for (const t of ['communications', 'communication_jobs', 'outbox', 'audit_events', 'commands', 'acknowledgements'])
...
assert.equal((await one(`select count(*)::int n from app.outbox_action_types where function_id in ('FN-20','FN-18')`)).n, 0);

// ---------------------------------------------------------------------------
// 1. A captured draft, exactly as the producers write one
// ---------------------------------------------------------------------------
const merchant = (await one(`insert into public.companies (name, type) values ('Acme Merchant','Merchant') returning id`)).id;
await db.query(`insert into public.contacts (company_id, name, email, preferred_channel) values ($1,'Pat','Pat@Acme.example','Email')`, [merchant]);
await db.query(`insert into public.contacts (company_id, name, email) values ($1,'No Address',null)`, [merchant]);

const draft = async (over = {}) => (await one(
  ...
  [job, merchant, over.type ?? 'MerchantOrder', over.subject ?? 'Order SS-0001 rev 1',
   over.body ?? 'CAPTURED DRAFT — not sent. FN-03 R2.',
   over.recipients ?? (await one(`select app.mat_contacts($1)::text r`, [merchant])).r])).id;

const c1 = await draft();
// The snapshot keeps the unusable contact; only the usable one is a recipient.
const rec = (await one(`select app.comm_recipients(c) r from public.communications c where c.id=$1`, [c1])).r;
assert.equal(rec.ok, true);
assert.deepEqual(rec.to, ['pat@acme.example'], 'lower-cased, usable only');
assert.equal(rec.unusable.length, 1, 'the contact with no address is reported, not silently dropped');

// ---------------------------------------------------------------------------
// 2. Approval: authority, then state
// ---------------------------------------------------------------------------
await refuses('inst_a', C('COMMUNICATION_APPROVE', { communication_id: c1 }), 'R1A_ROLE_DENIED');
await refuses('store', C('COMMUNICATION_APPROVE', { communication_id: c1 }), 'R1A_ROLE_DENIED');
await refuses('sam', C('COMMUNICATION_APPROVE', { communication_id: c1 }), 'R1A_ROLE_DENIED');
await refuses('tanya', C('COMMUNICATION_APPROVE', {}), 'COMM_REFUSED: communication_id is required');
await refuses('tanya', C('COMMUNICATION_APPROVE', { communication_id: id() }), 'COMM_NOT_FOUND');
await refuses('tanya', C('COMMUNICATION_APPROVE', { communication_id: c1 }, { expected_version: 99 }), 'R1A_STALE_VERSION');

let r = ok(await cmd('tanya', C('COMMUNICATION_APPROVE', { communication_id: c1, reason: 'checked the lines' })), 'approve');
...
assert.equal((await one(`select count(*)::int n from public.audit_events where entity_id=$1 and action='CommunicationApproved' and initiating_person_id=$2`, [c1, people.tanya])).n, 1);

// Idempotent, and never a second audit row.
r = ok(await cmd('tanya', C('COMMUNICATION_APPROVE', { communication_id: c1 })), 're-approve');
assert.equal(r.already_approved, true);
assert.equal((await one(`select count(*)::int n from public.audit_events where entity_id=$1 and action='CommunicationApproved'`, [c1])).n, 1);

// A Director may approve but may not queue.
const cDir = await draft();
ok(await cmd('dan', C('COMMUNICATION_APPROVE', { communication_id: cDir })), 'director approves');
await refuses('dan', C('COMMUNICATION_QUEUE', { communication_id: cDir }), 'R1A_ROLE_DENIED');

// ---------------------------------------------------------------------------
// 3. Door 1: the release mode. With FN-03 Disabled, queueing writes NOTHING.
// ---------------------------------------------------------------------------
await refuses('tanya', C('COMMUNICATION_QUEUE', { communication_id: c1 }), 'R1A_MODE_DENIED');
assert.equal((await one(`select count(*)::int n from public.outbox`)).n, 0, 'no dispatch intent may accumulate while Disabled');

// A type that is never dispatchable stays refused whatever the modes say.
const notice = await draft({ type: 'CustomerNotice' });
ok(await cmd('tanya', C('COMMUNICATION_APPROVE', { communication_id: notice })), 'approve notice');
await refuses('tanya', C('COMMUNICATION_QUEUE', { communication_id: notice }), 'COMM_NOT_DISPATCHABLE');

// ---------------------------------------------------------------------------
// 4. The manual route (FN-20 / FN-18): record the fact, claim no delivery
// ---------------------------------------------------------------------------
await refuses('tanya', C('COMMUNICATION_RECORD_SENT', { communication_id: notice }),
  'COMM_REFUSED: note is required - say in your own words what was sent and to whom');
await refuses('tanya', C('COMMUNICATION_RECORD_SENT', { communication_id: notice, note: 'x', sent_at: '2099-01-01' }),
  'COMM_REFUSED: sent_at is in the future');
r = ok(await cmd('tanya', C('COMMUNICATION_RECORD_SENT',
  { communication_id: notice, note: 'Emailed Ann from my own mailbox', external_reference: 'gmail-thread-7' })), 'record sent');
assert.equal(r.recorded, true);
assert.equal(r.sent_by_system, false);
row = await commRow(notice);
assert.equal(row.status, 'Sent');
assert.equal(row.external_message_id, 'manual:gmail-thread-7', 'a human reference is never mistaken for a transport id');
assert.equal(row.outbox_id, null, 'the manual route creates no outbox row');
assert.equal((await one(`select count(*)::int n from public.audit_events where entity_id=$1 and action='CommunicationRecordedSentByPerson' and reason='Emailed Ann from my own mailbox'`, [notice])).n, 1);
// Not twice.
await refuses('tanya', C('COMMUNICATION_RECORD_SENT', { communication_id: notice, note: 'again' }), 'COMM_STATUS_INVALID');

// ---------------------------------------------------------------------------
// 5. Door 1 open: queueing now works, and still sends nothing
// ---------------------------------------------------------------------------
await db.query(`update public.release_modes set mode='Automated', authorised_job_scope='Pilot' where function_id in ('FN-03','FN-04')`);
r = ok(await cmd('tanya', C('COMMUNICATION_QUEUE', { communication_id: c1 })), 'queue');
...
r = ok(await cmd('tanya', C('COMMUNICATION_QUEUE', { communication_id: c1 })), 're-queue');
...
await refuses('tanya', C('COMMUNICATION_QUEUE', { communication_id: c2 }), 'COMM_NOT_APPROVED');

// A message with no usable address can be approved but never queued.
const noAddr = await draft({ recipients: '[{"contact_id":"x","name":"Nobody","email":"NOT_CONFIGURED"}]' });
ok(await cmd('tanya', C('COMMUNICATION_APPROVE', { communication_id: noAddr })), 'approve no-address');
await refuses('tanya', C('COMMUNICATION_QUEUE', { communication_id: noAddr }), 'COMM_RECIPIENTS_INVALID');

// ---------------------------------------------------------------------------
// 6. Door 4: the allow-list. An empty one refuses the whole claim.
// ---------------------------------------------------------------------------
await setSetting('email.mode', 'LIVE');
assert.equal(await sqlErr(`select app.outbox_claim(array['EmailOrder'], 10) r`),
  'EMAIL_REFUSED: LIVE mode requires outbound.allowed_recipients');
assert.equal((await outRow(o1)).status, 'Pending', 'a refused claim leaves the row untouched');
...
assert.equal(claim.settled[0].code, 'SENDER_NOT_CONFIGURED', JSON.stringify(claim.settled[0]));
out = await outRow(o1);
assert.equal(out.status, 'NeedsReview');
assert.match(out.response_summary, /SENDER_NOT_CONFIGURED|email\.from_mailbox/);
assert.equal((await commRow(c1)).status, 'Uncertain', 'the message follows its outbox row into review');
...
ok(await cmd('tanya', C('COMMUNICATION_APPROVE', { communication_id: c3 })), 'approve c3');
const o3 = ok(await cmd('tanya', C('COMMUNICATION_QUEUE', { communication_id: c3 })), 'queue c3').outbox_id;
claim = await sql(`select app.outbox_claim(array['EmailOrder'], 10) r`);
assert.equal(claim.claimed.length, 0);
assert.ok(claim.skipped.some(s => s.outbox_id === o3 && s.reason === 'CAPTURE_MODE'), JSON.stringify(claim));
assert.equal((await outRow(o3)).status, 'Pending', 'CAPTURE reports and leaves it Pending');
...
assert.equal(out.attempt_count, 1, 'the attempt is recorded BEFORE any external call');
assert.ok(out.claimed_at);
assert.equal((await commRow(c3)).status, 'Queued', 'claiming does not change the message');

// A transport result lands through the recorded paths only.
await sql(`select app.outbox_record_success($1, 'gmail-msg-1', 'sent') r`, [o3]);
out = await outRow(o3);
...
assert.equal((await one(`select count(*)::int n from public.audit_events where entity_id=$1 and action='CommunicationDispatchSucceeded'`, [c3])).n, 1);
assert.equal((await one(`select executing_service s from public.audit_events where entity_id=$1 and action='CommunicationDispatchSucceeded'`, [c3])).s, 'EmailService',
...
// ---------------------------------------------------------------------------
await db.query(`update public.communications set status='Queued' where id=$1`, [c3]);
await db.query(`update public.outbox set status='Pending', next_attempt=now(), attempt_count=0, claimed_at=null where id=$1`, [o3]);
claim = await sql(`select app.outbox_claim(array['EmailOrder'], 10) r`);
assert.equal(claim.claimed.length, 0, 'an already-sent message is never sent again');
const settled3 = claim.settled.find(s => s.outbox_id === o3);
assert.ok(settled3, 'the re-queued row was settled, not left pending: ' + JSON.stringify(claim));
...
ok(await cmd('tanya', C('COMMUNICATION_APPROVE', { communication_id: c4 })), 'approve c4');
const o4 = ok(await cmd('tanya', C('COMMUNICATION_QUEUE', { communication_id: c4 })), 'queue c4').outbox_id;
// Someone edits the body after approval (a future editor, or a data fix).
await db.query(`update public.communications set subject='Order SS-0001 rev 1 URGENT' where id=$1`, [c4]);
claim = await sql(`select app.outbox_claim(array['EmailOrder'], 10) r`);
assert.equal(claim.claimed.length, 0, 'changed content is never sent on the old approval');
assert.equal(claim.settled.find(s => s.outbox_id === o4).code, 'PAYLOAD_CHANGED', JSON.stringify(claim.settled));
...
ok(await cmd('tanya', C('COMMUNICATION_APPROVE', { communication_id: c5 })), 'approve c5');
const o5 = ok(await cmd('tanya', C('COMMUNICATION_QUEUE', { communication_id: c5 })), 'queue c5').outbox_id;
await setSetting('outbound.allowed_recipients', ['someone.else@example.com']);
claim = await sql(`select app.outbox_claim(array['EmailOrder'], 10) r`);
assert.equal(claim.claimed.length, 0);
assert.equal(claim.settled.find(s => s.outbox_id === o5).code, 'RECIPIENT_NOT_ALLOWLISTED', JSON.stringify(claim.settled));
...
assert.equal(d.data.communications.find(c => c.communication_id === notice).dispatchable, false);

d = await opread('tanya', { read_type: 'COMMUNICATIONS', job_id: job, status: 'Uncertain' });
assert.ok(d.data.communications.every(c => c.status === 'Uncertain'));
assert.equal((await opread('tanya', { read_type: 'COMMUNICATIONS', nonsense: 1 })).error, 'R1A_INVALID_FIELDS');
...
for (const who of ['inst_a', 'sam', 'store'])
  assert.equal((await opread(who, { read_type: 'COMMUNICATIONS', job_id: job })).error, 'R1A_ROLE_DENIED', who);
