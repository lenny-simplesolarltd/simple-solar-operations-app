...
const job = await one(`select id, job_ref from public.jobs where id=$1`, [jobId]);

const C = (type, payload) => ({ command_id: id(), command_type: type, payload });
const opread = async (who, req) => {
  await as(who);
  try {
    return (await db.query(`select public.execute_operations_read($1::jsonb) r`, [req])).rows[0].r;
  } catch (e) {
...
const asUser = async (who, fn) => {
  ...
  try { return await fn(); }
  catch (e) { throw new Error(`as ${who}: ${e.message}`); }
  finally { await db.query('reset role'); }
};
const visibleMessages = (who) =>
  asUser(who, async () => (await all(`select id from public.chat_messages`)).length);
...
for (const t of ['CHAT_START', 'CHAT_SEND', 'CHAT_EDIT', 'CHAT_DELETE', 'CHAT_REACT', 'CHAT_MARK_READ'])
  assert.ok(types.includes(t), 'registered: ' + t);

// ---------------------------------------------------------------------------
// 1. Direct conversations are unique per pair, whoever opens them
// ---------------------------------------------------------------------------
let r = ok(await cmd('tanya', C('CHAT_START', { kind: 'Direct', person_ids: [people.ben] })), 'start');
...
r = ok(await cmd('ben', C('CHAT_START', { kind: 'Direct', person_ids: [people.tanya] })), 'reopen');
assert.equal(r.created, false);
assert.equal(r.conversation_id, direct, 'the pair has exactly one direct conversation');
assert.equal((await one(`select count(*)::int n from public.chat_conversations`)).n, 1);
...
r = ok(await cmd('tanya', C('CHAT_SEND', { conversation_id: direct, body: 'and SS-ZZZZ-9999 too' })), 'unknown ref');
...
assert.deepEqual(r.mentioned, [people.ben], 'only actual members are mentioned');
...
await refuses('dan', C('CHAT_SEND', { conversation_id: direct, body: 'hello' }), 'CHAT_NOT_A_MEMBER');

let read = await opread('dan', { read_type: 'CHAT_MESSAGES', conversation_id: direct });
assert.match(read.error ?? '', /CHAT_NOT_A_MEMBER/, 'a non-member cannot read the thread');
...
assert.equal(await visibleMessages('dan'), 0, 'RLS hides the messages from a non-member');
...
await db.query(`insert into public.person_roles (person_id, role_code) values ($1,'Admin') on conflict do nothing`, [people.dan]);
await asUser('dan', async () => {
  assert.ok((await all(`select id from public.chat_conversations`)).length >= 1,
    'a chat admin lists conversations');
  assert.ok((await all(`select id from public.chat_members`)).length >= 1,
    'and sees who is in them');
  assert.equal((await all(`select id from public.chat_messages`)).length, 0,
...
ok(await cmd('ben', C('CHAT_MARK_READ', { conversation_id: direct })), 'mark read');
read = await opread('ben', { read_type: 'CHAT_CONVERSATIONS' });
...
ok(await cmd('ben', C('CHAT_SEND', { conversation_id: direct, body: 'on it' })), 'ben replies');
read = await opread('ben', { read_type: 'CHAT_CONVERSATIONS' });
conv = read.data.conversations.find(c => c.conversation_id === direct);
...
const before = (await one(`select last_read_at from public.chat_members where conversation_id=$1 and person_id=$2`, [direct, people.ben])).last_read_at;
ok(await cmd('ben', C('CHAT_MARK_READ', { conversation_id: direct, at: '2020-01-01T00:00:00Z' })), 'backdated');
const after = (await one(`select last_read_at from public.chat_members where conversation_id=$1 and person_id=$2`, [direct, people.ben])).last_read_at;
assert.deepEqual(after, before, 'a backdated mark-read is ignored');

// ---------------------------------------------------------------------------
// 5. Editing and deleting
// ---------------------------------------------------------------------------
await refuses('ben', C('CHAT_EDIT', { message_id: first, body: 'changed' }), 'CHAT_NOT_YOUR_MESSAGE');

ok(await cmd('tanya', C('CHAT_EDIT', { message_id: first, body: 'Can you check it today?' })), 'edit');
...
assert.match(row.original_body, /Can you check SS-/, 'the first edit keeps what was originally said');

// A second edit does not overwrite the original.
ok(await cmd('tanya', C('CHAT_EDIT', { message_id: first, body: 'third wording' })), 'edit again');
row = await one(`select * from public.chat_messages where id=$1`, [first]);
assert.match(row.original_body, /Can you check SS-/, 'still the FIRST body, not the second');

// Deleting keeps the row so the conversation keeps its shape.
ok(await cmd('tanya', C('CHAT_DELETE', { message_id: first, reason: 'wrong thread' })), 'delete');
row = await one(`select * from public.chat_messages where id=$1`, [first]);
assert.equal(row.body, '');
assert.ok(row.deleted_at);
assert.equal((await one(`select count(*)::int n from public.chat_messages where conversation_id=$1`, [direct])).n, 4,
  'the message is still there, blanked');
assert.equal((await one(`select count(*)::int n from public.audit_events where action='ChatMessageDeleted'`)).n, 1,
...
assert.equal((await one(`select count(*)::int n from public.audit_events where action like 'ChatMessage%' and action <> 'ChatMessageDeleted'`)).n, 0);

// ---------------------------------------------------------------------------
// 6. Reactions
// ---------------------------------------------------------------------------
const bensMsg = (await one(`select id from public.chat_messages where author_person_id=$1 order by created_at desc limit 1`, [people.ben])).id;
ok(await cmd('tanya', C('CHAT_REACT', { message_id: bensMsg, emoji: '👍' })), 'react');
assert.equal((await one(`select count(*)::int n from public.chat_reactions`)).n, 1);
// Reacting twice with the same emoji is idempotent, not a second reaction.
ok(await cmd('tanya', C('CHAT_REACT', { message_id: bensMsg, emoji: '👍' })), 'react again');
assert.equal((await one(`select count(*)::int n from public.chat_reactions`)).n, 1);
ok(await cmd('tanya', C('CHAT_REACT', { message_id: bensMsg, emoji: '👍', on: false })), 'unreact');
assert.equal((await one(`select count(*)::int n from public.chat_reactions`)).n, 0);
// A non-member cannot react to a message they cannot see.
await refuses('dan', C('CHAT_REACT', { message_id: bensMsg, emoji: '👍' }), 'CHAT_NOT_A_MEMBER');

// ---------------------------------------------------------------------------
// 7. Groups, and who may be in one
// ---------------------------------------------------------------------------
r = ok(await cmd('tanya', C('CHAT_START', {
  kind: 'Group', title: 'Thursday installs', person_ids: [people.ben, people.dan]
})), 'group');
...
await refuses('tanya', C('CHAT_START', { kind: 'Direct', person_ids: [people.ben, people.dan] }),
...
const mute = (await one(`insert into public.people (legacy_id, email, display_name) values ('PERSON-mute','mute@test.local','Mute') returning id`)).id;
await db.query(`insert into public.person_roles (person_id, role_code) values ($1,'ReadOnly')`, [mute]);
await refuses('tanya', C('CHAT_START', { kind: 'Group', person_ids: [people.ben, mute] }), 'CHAT_MEMBER_CANNOT_CHAT');

// An inactive person likewise.
await db.query(`update public.people set active=false where id=$1`, [people.sam]);
await refuses('tanya', C('CHAT_START', { kind: 'Group', person_ids: [people.ben, people.sam] }), 'CHAT_MEMBER_INVALID');

// ---------------------------------------------------------------------------
// 8. Replies stay inside their conversation
// ---------------------------------------------------------------------------
r = ok(await cmd('tanya', C('CHAT_SEND', { conversation_id: group, body: 'morning' })), 'group message');
const groupMsg = r.message_id;
await refuses('tanya', C('CHAT_SEND', {
  conversation_id: direct, body: 'reply across', reply_to_id: groupMsg
