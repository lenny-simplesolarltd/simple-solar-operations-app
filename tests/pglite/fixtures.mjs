...
export async function setup() {
  ...
  for (const [key, name, role, capacity] of [
    ['tanya', 'Tanya', 'Office'],
    ['hannah', 'Hannah', 'VariationApprover'],
    ['ben', 'Ben Quick', 'Admin'],
    ['dan', 'Dan', 'Director'],
    ['store', 'Store Owner', 'Store'],
    ['sam', 'Sam Surveyor', 'Surveyor'],
    ['inst_a', 'Installer A', 'Installer', 1],
    ['inst_b', 'Installer B', 'Installer', 1]
  ]) {
    const email = `${key}@test.local`;
    const p = await one(
      `insert into public.people (legacy_id, email, display_name, capacity_per_day) values ($1,$2,$3,$4) returning id`,
      [`PERSON-${key}`, email, name, capacity ?? null]
    );
    await db.query(
      `insert into public.person_roles (person_id, role_code) values ($1,$2)`,
      [p.id, role]
    );
  ...
  await db.query(
    `insert into public.task_assignment_rules (template_code, owner_person_id, backup_person_id, eligible_owner_roles)
    select t.code,
  ...
  async function as(who) {
    const claims =
      who && users[who]
        ? { sub: users[who], email: `${who}@test.local`, role: 'authenticated' }
        : {};
  ...
  async function cmd(who, request) {
    ...
    try {
      return (
        await db.query(`select public.execute_command($1::jsonb) r`, [request])
      ).rows[0].r;
    } catch (e) {
  ...
  async function read(who, request) {
    ...
    try {
      return (
        await db.query(`select public.execute_read($1::jsonb) r`, [request])
      ).rows[0].r;
    } catch (e) {
      return { error: e.message, detail: e.detail };
    }
...
export async function readyToBook(f, over = {}) {
  ...
  ok(
    await cmd('tanya', {
      ...
      payload: {
        ...
        evidence_path: `${job}/contract.pdf`
      }
    }),
    'pre02'
  );
  ...
  if (r.job.workflow_stage !== 'ReadyToBook') {
    console.log('not ready', JSON.stringify(r.readiness));
    process.exit(1);
  }
