...
assert.ok(
  r.tasks.length > 0 &&
    r.tasks.every(
      (t) => t.owner_id === people.tanya || t.backup_id === people.tanya
    )
);
assert.ok(
  r.tasks.every(
    (t) => !['Complete', 'Cancelled', 'NotRequired'].includes(t.status)
  )
);
// AppSheet parity: open undated non-booking tasks are included (MY_TASKS dropped them).
const pre04 = r.tasks.find(
  (t) => t.template_code === 'PRE04' && t.job_id === sold2.job_id
);
assert.ok(pre04, 'undated PRE04 on the second job is in My Tasks');
assert.equal(pre04.job_ref, sold2.job_ref);
assert.equal(pre04.customer_name, 'Ann Jones');
assert.equal(pre04.postcode, 'YO1 2BB');

r = data(
  await ops('tanya', { read_type: 'TASKS', status: 'closed' }),
  'history'
);
...
assert.ok(
  r.tasks.length > 0 && r.tasks.every((t) => t.job_id === sold2.job_id)
);
r = data(
  await ops('tanya', { read_type: 'TASKS', q: 'yo12bb' }),
...
assert.ok(
  r.tasks.some((t) => t.template_code === 'PRE03'),
  'PRE03 (ben) in tanya team view'
);
...
assert.ok(
  r.tasks.every((t) => t.owner_id === people.ben || t.backup_id === people.ben)
);

r = data(
  await ops('tanya', { read_type: 'TASKS', scope: 'all', job_id: jobRef }),
  'job filter by ref'
);
assert.ok(r.tasks.every((t) => t.job_id === job));

r = data(
  await ops('tanya', { read_type: 'TASKS', scope: 'all', due: 'none' }),
  'due none'
);
...
assert.equal(
  (await ops('sam', { read_type: 'TASKS', scope: 'team' })).error,
  'R1A_ROLE_DENIED'
);
assert.equal(
  (await ops('tanya', { read_type: 'TASKS', bogus: 1 })).error,
  'R1A_INVALID_FIELDS'
);
assert.equal(
  (await ops('tanya', { read_type: 'TASKS', queue: 'nope' })).error,
  'R1A_INVALID_FIELDS'
);
assert.equal(
  (await ops(null, { read_type: 'TASKS' })).error,
  'R1A_AUTHENTICATED_EMAIL_REQUIRED'
);

// Booking queue = tasks in group Booking/Prebooking (reference rule), not a stage guess.
r = data(
  await ops('tanya', { read_type: 'TASKS', scope: 'all', queue: 'booking' }),
  'booking queue'
);
assert.ok(
  r.tasks.length > 0 &&
    r.tasks.every((t) => ['Booking', 'Prebooking'].includes(t.group))
);

// ---- TASK_DETAIL --------------------------------------------------------
const pre03 = await one(
  `select * from public.tasks where job_id=$1 and template_code='PRE03'`,
  [sold2.job_id]
);
r = data(
  await ops('ben', { read_type: 'TASK_DETAIL', task_id: pre03.id }),
  'task detail'
);
assert.equal(r.task.id, pre03.id);
assert.equal(r.task.is_mine, true);
assert.equal(r.job.job_ref, sold2.job_ref);
...
assert.equal(
  (await ops('inst_a', { read_type: 'TASK_DETAIL', task_id: pre03.id })).error,
...
assert.equal(j2.customer_name, 'Ann Jones');
assert.equal(j2.workflow_stage, 'Prebooking');
assert.ok(j2.open_tasks > 0 && j2.next_task && j2.next_task.title);
r = data(
  await ops('tanya', { read_type: 'JOBS', stage: 'ReadyToBook' }),
  'stage filter'
);
assert.deepEqual(
  r.jobs.map((x) => x.id),
...
r = data(
  await ops('tanya', { read_type: 'JOBS', q: jobRef.toLowerCase() }),
  'ref search'
);
assert.deepEqual(
  r.jobs.map((x) => x.id),
...
assert.equal(
  (await ops('tanya', { read_type: 'JOBS', stage: 'Nope' })).error,
  'R1A_INVALID_FIELDS'
);
...
assert.equal(
  (await ops('inst_a', { read_type: 'JOBS' })).error,
  'R1A_ROLE_DENIED'
);
...
const ov = (
  await db.query(`select public.execute_read($1::jsonb) r`, [
    { read_type: 'JOB_OVERVIEW', job_id: job }
  ])
).rows[0].r;
...
assert.equal(r.jobs_by_stage.ReadyToBook, 1);
assert.equal(r.jobs_by_stage.Prebooking, 1);
assert.equal(r.can_view_team, true);
assert.ok(r.operations.booking_queue > 0);
r = data(await ops('sam', { read_type: 'OFFICE_DASHBOARD' }), 'sam dashboard');
assert.equal(r.can_view_team, false);
assert.equal(r.operations.booking_queue, null);

// ---- MY_REQUESTS --------------------------------------------------------
r = data(await ops('tanya', { read_type: 'MY_REQUESTS' }), 'requests');
assert.ok(r.count >= 3);
assert.ok(
  r.requests.some((x) => x.command_type === 'SOLD_INTAKE' && x.job_ref)
);
assert.ok(
  r.requests.some(
    (x) =>
      x.command_type === 'TASK_COMPLETE' &&
      x.outcome &&
      x.outcome.status === 'Succeeded'
  )
);
r = data(await ops('dan', { read_type: 'MY_REQUESTS' }), 'dan requests');
assert.ok(
  r.requests.every((x) => x.command_type !== 'SOLD_INTAKE'),
  'only own commands'
);
