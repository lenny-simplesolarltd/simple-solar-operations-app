...
test('changes several contact fields at once', async () => {
    ...
    await cmd(
      ...
      C(
        'CUSTOMER_UPDATE',
        {
          ...
          contact_notes: 'Prefers calls after 6pm'
        },
        { expected_version: before.version }
      )
    ),
  ...
  assert.equal(
    after.phone,
    before.phone,
    'phone was not named, so phone did not change'
  );
});
...
test("refuses to change the customer's name or address", async () => {
  ...
  for (const payload of [
    ...
    const code = await refuses(
      'tanya',
      C('CUSTOMER_UPDATE', payload, { expected_version: v })
    );
    assert.match(
      code,
      /R1A_INVALID_FIELDS/,
...
test('refuses to change the agreed commercial terms', async () => {
  ...
  for (const payload of [
    ...
    const code = await refuses(
      'tanya',
      C('JOB_SALE_UPDATE', payload, { expected_version: j.version })
    );
    assert.match(
      code,
      /R1A_INVALID_FIELDS/,
...
test('refuses to leave a customer with no way of being contacted', async () => {
  ...
  const code = await refuses(
    'tanya',
    C('CUSTOMER_UPDATE', { phone: '', email: '' }, { expected_version: v })
  );
...
test('clears one contact detail while the other still reaches them', async () => {
  ...
  ok(
    await cmd(
      'tanya',
      C('CUSTOMER_UPDATE', { phone: '' }, { expected_version: before.version })
    ),
    'clear phone'
  );
  const after = await customer();
...
test('refuses a value that is not a contact detail', async () => {
  ...
  assert.match(
    await refuses(
      'tanya',
      C('CUSTOMER_UPDATE', { phone: 'give me a ring' }, { expected_version: v })
    ),
  ...
  assert.match(
    await refuses(
      'tanya',
      C('CUSTOMER_UPDATE', { email: 'not-an-address' }, { expected_version: v })
    ),
...
test('refuses a correction proposed from stale details', async () => {
  ...
  const code = await refuses(
    'tanya',
    C('CUSTOMER_UPDATE', { phone: '01752 000111' }, { expected_version: v - 1 })
  );
...
test('refuses a change that changes nothing', async () => {
  ...
  assert.match(
    await refuses(
      'tanya',
      C('CUSTOMER_UPDATE', { phone: c.phone }, { expected_version: c.version })
    ),
    /CUSTOMER_NO_CHANGE/
  );
  const j = await job();
...
test('refuses staff who do not hold the permission', async () => {
  ...
  const code = await refuses(
    'inst_a',
    C('CUSTOMER_UPDATE', { phone: '01752 000222' }, { expected_version: v })
  );
...
test('refuses an imported historical record', async () => {
  ...
  const code = await refuses(
    'tanya',
    C('CUSTOMER_UPDATE', { phone: '01752 000333' }, { expected_version: v })
  );
...
test('replaying the same command does not apply it twice', async () => {
  ...
  assert.equal(
    after.version,
    before.version + 1,
    'the version moved once, not twice'
  );
});
...
test('every accepted change is on the audit trail', async () => {
  ...
  const before = (
    await all(`select count(*)::int n from public.audit_events`)
  )[0].n;
  await refuses(
    'tanya',
    C(
      'CUSTOMER_UPDATE',
      { phone: 'nope' },
      { expected_version: (await customer()).version }
    )
  );
  const after = (
    await all(`select count(*)::int n from public.audit_events`)
