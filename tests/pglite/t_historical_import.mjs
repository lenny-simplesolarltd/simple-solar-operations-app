...
async function historicalJob(overrides = {}) {
  ...
  return one(sql, keys.map((k) => cols[k]));
}
...
test('1. a historical job insert creates zero invoice stages', async () => {
  const j = await historicalJob({ original_gross_pence: 1500000, current_contract_gross_pence: 1500000 });
  const r = await one(`select count(*)::int n from public.invoice_stages where job_id = $1`, [j.id]);
  assert.equal(r.n, 0, 'a historical job must not acquire a financial obligation');
});

test('1b. a live job insert still creates its invoice stages', async () => {
  const j = await liveJobInsert();
  const r = await one(`select count(*)::int n from public.invoice_stages where job_id = $1`, [j.id]);
  assert.ok(r.n >= 2, `live behaviour must be unchanged; expected >= 2 stages, got ${r.n}`);
});

test('2. a historical job insert creates zero tasks', async () => {
  const j = await historicalJob();
  const r = await one(`select count(*)::int n from public.tasks where job_id = $1`, [j.id]);
  assert.equal(r.n, 0);
});
...
test('3. a historical job is not actionable', async () => {
  const j = await historicalJob();
  const r = await one(`select app.job_actionable(j) a, app.job_in_scope(j) s from public.jobs j where j.id = $1`, [j.id]);
  assert.equal(r.a, false, 'historical jobs must never be actionable');
  assert.equal(r.s, false, 'historical jobs must never be in operational scope');
});
...
test('3c. a historical job cannot be un-archived to sneak back into scope', async () => {
  const j = await historicalJob();
  await refuses(
    () => db.query(`update public.jobs set archived_at = null where id = $1`, [j.id]),
...
test('5. a historical job with an unknown salesperson is accepted', async () => {
  ...
  const r = await one(`select salesperson_id from public.jobs where id = $1`, [j.id]);
  assert.equal(r.salesperson_id, null, 'unknown must stay unknown, not become a stand-in person');
});

test('6. a live job without a salesperson is still refused', async () => {
  await refuses(() => liveJobInsert({ salesperson_id: null }), 'jobs_live_requires_sale_fields');
});

test('7. a historical job with unknown finance is accepted', async () => {
  const j = await historicalJob({ finance_route: null });
  const r = await one(`select finance_route from public.jobs where id = $1`, [j.id]);
  assert.equal(r.finance_route, null);
});

test('8. a live job without a finance route is still refused', async () => {
  await refuses(() => liveJobInsert({ finance_route: null }), 'jobs_live_requires_sale_fields');
});

test('8b. an invalid finance route is still refused on a historical job', async () => {
  await refuses(() => historicalJob({ finance_route: 'MadeUpRoute' }), 'finance_route');
});

test('9. a historical job with an unknown price is accepted', async () => {
  const j = await historicalJob({ original_gross_pence: null, current_contract_gross_pence: null });
  const r = await one(`select original_gross_pence from public.jobs where id = $1`, [j.id]);
  assert.equal(r.original_gross_pence, null);
});

test('10. a live job with a missing price is still refused', async () => {
  await refuses(() => liveJobInsert({ original_gross_pence: null }), 'jobs_live_requires_sale_fields');
});

test('10b. a zero price is still refused, on both classes', async () => {
  await refuses(() => liveJobInsert({ original_gross_pence: 0 }), 'original_gross_pence');
  await refuses(() => historicalJob({ original_gross_pence: 0 }), 'original_gross_pence');
});

test('10c. a historical job must declare where it came from', async () => {
  await refuses(() => historicalJob({ source_system: null }), 'jobs_historical_is_inert');
});
...
test('11. an unresolved historical installer is preserved without an allocation', async () => {
  ...
  assert.equal(kept.source_value, 'Des', 'the original text must survive verbatim');
  assert.equal(kept.person_id, null, 'an unresolved name must not be linked to a person');
  ...
  assert.equal(allocations.n, 0, 'a historical name must never become live resourcing');
});
...
test('11c. a resolved historical name may be linked, still without an allocation', async () => {
  ...
  const r = await one(`select person_id from public.historical_job_people where job_id = $1`, [j.id]);
...
test('12. two rows sharing a Submission ID stay independently addressable', async () => {
  ...
  assert.equal(r.n, 2, 'both source rows must be importable, not one of them dropped');
});
...
test('14. historical jobs remain findable in Job Search', async () => {
  ...
  const admin = await one(`select app.read_job_search(
  ...
  assert.equal(results.find((x) => x.job_ref === ref).record_class, 'HistoricalImport',
...
test('15. the legacy reference is searchable too', async () => {
  const admin = await one(`select app.read_job_search(
    ...
    [person.id]);
  assert.ok((admin.r.results ?? []).length >= 1,
...
test('16. historical jobs create no work package, material, scaffold or commissioning row', async () => {
  const j = await historicalJob();
  for (const table of ['work_packages', 'materials', 'scaffold_bookings', 'commissioning_submissions', 'orders']) {
    const r = await one(`select count(*)::int n from public.${table} where job_id = $1`, [j.id]);
    assert.equal(r.n, 0, `${table} must stay empty for a historical job insert`);
  }
});

test('17. historical jobs create no communication, outbox, call or GHL side effect', async () => {
  const before = {};
  for (const t of ['communications', 'outbox', 'calls', 'ghl_tasks', 'invoice_stages', 'payments']) {
    before[t] = (await one(`select count(*)::int n from public.${t}`)).n;
  }
  await historicalJob();
  for (const t of ['communications', 'outbox', 'calls', 'ghl_tasks', 'invoice_stages', 'payments']) {
    const after = (await one(`select count(*)::int n from public.${t}`)).n;
    assert.equal(after, before[t], `${t} must be untouched by a historical job insert`);
  }
});
...
test('18. the apply function is idempotent on a second call', async () => {
  ...
  const doc = {
    provenance: {
      formId: 'historical-job-booking-form', importKey: 'APPLY-IDEM-1',
      submissionId: 'APPLY-IDEM-1', payloadHash: 'hash-a',
      legacyReference: 'LEG-1', submittedAt: '2025-03-07T09:48:24Z',
      sourceSystem: 'historical-job-booking-form', batchId: 'testbatch',
    },
    customer: {
      firstName: 'Pat', lastName: 'Quinn', addressLine1: '1 Test Lane',
      addressLine2: null, town: 'Testville', postcode: 'ZZ4 4ZZ',
      email: 'apply.idem@test.invalid', phone: null,
    },
    job: {
      soldAt: '2025-03-07T09:48:24Z', grossPence: null, leadSource: null,
      financeRoute: null, roofRequired: true, electricalRequired: true,
      scaffoldRequired: false, salespersonLegacyId: null,
    },
    technical: { systemKw: 4.5, mpan: null, roofNotes: null, electricalNotes: null },
    historicalPeople: [
      { role: 'Installer', sourceValue: 'Des', personLegacyId: null, matchKind: 'NoMatch', sourceColumn: 7 },
    ],
  };
  void c;

  const first = await one(`select app.historical_import_apply($1::jsonb, false) r`, [JSON.stringify(doc)]);
  assert.equal(first.r.action, 'imported');

  const second = await one(`select app.historical_import_apply($1::jsonb, false) r`, [JSON.stringify(doc)]);
  ...
  assert.equal(people.n, 1, 'a second apply must not duplicate historical staff');
});

test('19. a changed source payload is reported, not silently reapplied', async () => {
  const base = {
    provenance: {
      formId: 'historical-job-booking-form', importKey: 'APPLY-CHANGED-1',
      submissionId: 'APPLY-CHANGED-1', payloadHash: 'hash-original',
      legacyReference: 'LEG-2', submittedAt: '2025-03-07T09:48:24Z',
      sourceSystem: 'historical-job-booking-form', batchId: 'testbatch',
    },
    customer: {
      firstName: 'Pat', lastName: 'Quinn', addressLine1: '2 Test Lane',
      addressLine2: null, town: 'Testville', postcode: 'ZZ5 5ZZ',
      email: 'apply.changed@test.invalid', phone: null,
    },
    job: {
      soldAt: '2025-03-07T09:48:24Z', grossPence: 1000, leadSource: null,
      financeRoute: 'Standard', roofRequired: true, electricalRequired: false,
      scaffoldRequired: false, salespersonLegacyId: null,
    },
    technical: {}, historicalPeople: [],
  };
  await db.query(`select app.historical_import_apply($1::jsonb, false)`, [JSON.stringify(base)]);

  const changed = { ...base, provenance: { ...base.provenance, payloadHash: 'hash-different' } };
  const r = await one(`select app.historical_import_apply($1::jsonb, false) r`, [JSON.stringify(changed)]);
  assert.equal(r.r.action, 'skipped');
  assert.equal(r.r.source_changed, true, 'a changed re-export must be reported');
  assert.equal(r.r.writes, 0, 'and must not be written over the existing record');
});

test('20. an applied historical job creates no invoice stage, task or call', async () => {
  const before = {};
  for (const t of ['invoice_stages', 'tasks', 'calls', 'outbox', 'communications', 'allocations', 'work_packages']) {
    before[t] = (await one(`select count(*)::int n from public.${t}`)).n;
  }
  const doc = {
    provenance: {
      formId: 'historical-job-booking-form', importKey: 'APPLY-SIDE-1',
      submissionId: 'APPLY-SIDE-1', payloadHash: 'h', legacyReference: 'LEG-3',
      submittedAt: '2025-03-07T09:48:24Z',
      sourceSystem: 'historical-job-booking-form', batchId: 'testbatch',
    },
    customer: {
      firstName: 'Pat', lastName: 'Quinn', addressLine1: '3 Test Lane',
      addressLine2: null, town: 'Testville', postcode: 'ZZ6 6ZZ',
      email: 'apply.side@test.invalid', phone: null,
    },
    job: {
      soldAt: '2025-03-07T09:48:24Z', grossPence: 9999999, leadSource: null,
      financeRoute: 'Standard', roofRequired: true, electricalRequired: true,
      scaffoldRequired: true, salespersonLegacyId: null,
    },
    technical: {}, historicalPeople: [],
  };
  await db.query(`select app.historical_import_apply($1::jsonb, false)`, [JSON.stringify(doc)]);

  for (const [t, n] of Object.entries(before)) {
    const after = (await one(`select count(*)::int n from public.${t}`)).n;
    assert.equal(after, n, `${t} must be untouched even with a large gross amount present`);
  }
});
...
test('21. the apply function is not reachable from the application roles', async () => {
  ...
  assert.equal(grants.length, 0, 'historical_import_apply must not be granted to application roles');
});
...
test('22. a historical job offers no action to take', async () => {
  ...
  const offered = Object.entries(commands)
    ...
    .map(([name]) => name);
  assert.deepEqual(offered, [], `a historical job must offer nothing; got ${offered.join(', ')}`);
});
...
test('23. a live job still offers actions, so 22 is not vacuous', async () => {
  ...
  const commands = r.r?.commands ?? {};
  assert.ok(Object.keys(commands).length > 0, 'a live job must be assessed at all');
});
...
test('24. the history of a historical job is readable and holds no invented work', async () => {
  ...
  assert.equal(r.r.issue_events ?? 0, 0, 'a historical job has no issue history');
});
