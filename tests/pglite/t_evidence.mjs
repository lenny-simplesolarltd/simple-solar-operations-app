...
const rpc = async (who, fn, arg, cast = 'jsonb') => {
  await as(who);
  try {
    return (await db.query(`select public.${fn}($1::${cast}) r`, [arg])).rows[0]
      .r;
  } catch (e) {
...
const complete = (who, evidenceId) =>
  rpc(who, 'evidence_upload_complete', evidenceId, 'uuid');
const open = (who, evidenceId) => rpc(who, 'evidence_open', evidenceId, 'uuid');
const list = (who, req) => rpc(who, 'list_evidence', req);
const put = (path, who, over = {}) =>
  db.query(
    `insert into storage.objects (bucket_id, name, owner_id, metadata) values ('evidence', $1, $2, $3)`,
    [
      path,
      over.owner ?? users[who],
      JSON.stringify({
        size: over.size ?? 1234,
        mimetype: over.mime ?? 'application/pdf',
        eTag: '"abc"'
      })
    ]
  );
const evidence = (evidenceId) =>
  one(`select * from public.evidence where id=$1`, [evidenceId]);
const task = (job, code) =>
  one(`select * from public.tasks where job_id=$1 and template_code=$2`, [
    job,
    code
  ]);
const audits = async (evidenceId) =>
  (
    await all(
      `select action from public.audit_events where entity_type='Evidence' and entity_id=$1 order by occurred_at, id`,
      [evidenceId]
    )
  ).map((a) => a.action);
const counts = async () =>
  JSON.stringify(
    await Promise.all(
      ['evidence', 'audit_events', 'task_events', 'commands'].map((t) =>
        one(`select count(*)::int n from public.${t}`)
      )
    )
  );
const upload = (ctxType, ctxId, over = {}) => ({
  upload_id: id(),
...
const sue = (
  await one(
    `insert into public.people (legacy_id, email, display_name) values ('PERSON-sue','sue@test.local','Sue Surveyor') returning id`
  )
).id;
await db.query(
  `insert into public.person_roles (person_id, role_code) values ($1,'Surveyor')`,
  [sue]
);
const soldA = await sell('tanya');
assert.ok(!soldA.error, JSON.stringify(soldA));
const soldB = await sell('tanya', { sale: { salesperson_id: sue } });
assert.ok(!soldB.error, JSON.stringify(soldB));
const jobA = soldA.job_id,
  jobB = soldB.job_id;
const pre02A = await task(jobA, 'PRE02'),
  pre02B = await task(jobB, 'PRE02');

// ---- file rules (before any database state is consulted) --------------------------
for (const [over, code] of [
  [{ mime_type: 'text/html', filename: 'x.html' }, 'EVIDENCE_TYPE_NOT_ALLOWED'],
  [
    { mime_type: 'image/svg+xml', filename: 'x.svg' },
    'EVIDENCE_TYPE_NOT_ALLOWED'
  ],
  [
    { mime_type: 'application/x-msdownload', filename: 'x.exe' },
    'EVIDENCE_TYPE_NOT_ALLOWED'
  ],
  [
    { mime_type: 'image/jpeg', filename: 'photo.exe' },
    'EVIDENCE_TYPE_NOT_ALLOWED'
  ], // type and name disagree
  ...
  assert.equal(
    (await begin('tanya', upload('Task', pre02A.id, over))).error,
    code,
    JSON.stringify(over)
  );
...
assert.equal(
  (await begin(null, upload('Task', pre02A.id))).error,
  'R1A_AUTHENTICATED_EMAIL_REQUIRED'
);
assert.equal(
  (await begin('store', upload('Task', pre02A.id))).error,
  'R1A_ROLE_DENIED'
);
assert.equal(
  (await begin('inst_a', upload('Task', pre02A.id))).error,
  'R1A_ROLE_DENIED'
);
assert.equal(
  (await begin('hannah', upload('Task', pre02A.id))).error,
  'R1A_JOB_ACCESS_DENIED'
); // office, not on the job
assert.equal(
  (await begin('dan', upload('Task', pre02A.id))).error,
  'R1A_TASK_ACCESS_DENIED'
); // on the job, not this task
assert.equal(
  (await begin('tanya', upload('Task', id()))).error,
  'R1A_TASK_NOT_FOUND'
);
assert.equal(
  (await begin('tanya', upload('Job', jobA, { category: 'Selfie' }))).error,
  'EVIDENCE_CATEGORY_INVALID'
);
assert.equal(
  (await begin('hannah', upload('Job', jobA, { category: 'Other' }))).error,
  'R1A_JOB_ACCESS_DENIED'
);
...
assert.equal(
  (await begin('tanya', upload('Task', pre02A.id))).error,
  'R1A_INACTIVE_ACTOR'
);
...
const req1 = upload('Task', pre02A.id, {
  filename: `..\\..\\${jobB}/Signed Contract (final).pdf`,
  category: 'Progress'
});
...
assert.equal(
  r.category,
  'Contract',
  'the task template decides the category, not the browser'
);
assert.equal(
  r.storage_path,
  `${jobA}/${r.evidence_id}/Signed-Contract-final.pdf`,
  'no traversal into another job'
);
const ev1 = r.evidence_id;
let e = await evidence(ev1);
assert.equal(e.job_id, jobA);
assert.equal(e.task_id, pre02A.id);
assert.equal(e.uploaded_by, people.tanya);
assert.equal(e.context_type, 'Task');
assert.equal(e.mime_type, 'application/pdf');
assert.equal(Number(e.size_bytes), 1234);
assert.equal(
  e.original_filename,
  `..\\..\\${jobB}/Signed Contract (final).pdf`
);
// retry: same registration, no second row, no second audit event
let before = await counts();
r = await begin('tanya', req1);
assert.equal(r.evidence_id, ev1);
assert.equal(r.replayed, true);
assert.equal(await counts(), before, 'a retried registration wrote');
assert.equal(
  (await begin('tanya', { ...req1, size_bytes: 99 })).error,
  'EVIDENCE_UPLOAD_CONFLICT'
);
assert.equal(
  (await begin('tanya', { ...req1, context_id: pre02B.id })).error,
  'EVIDENCE_UPLOAD_CONFLICT'
);
assert.equal(
  (await begin('ben', { ...req1 })).error,
  'EVIDENCE_UPLOAD_CONFLICT',
  'an upload id belongs to one person'
);

// ---- required evidence before completion: a missing file completes nothing ---------------
const signed = (t, path, extra = {}) => ({
  command_id: id(),
...
before = await counts();
assert.equal(
  (await cmd('tanya', signed(pre02A, e.storage_path))).error,
  'R1A_UPLOAD_MISSING'
);
...
const canStore = async (who, path) => {
  await as(who);
  return (await one(`select app.evidence_can_store_object($1) ok`, [path])).ok;
};
const canRead = async (who, path) => {
  await as(who);
  return (await one(`select app.evidence_can_read_object($1) ok`, [path])).ok;
};
assert.equal(await canStore('tanya', e.storage_path), true);
assert.equal(
  await canStore('ben', e.storage_path),
  false,
  'Admin is not the uploader'
);
assert.equal(
  await canStore('tanya', `${jobA}/guessed.pdf`),
  false,
  'an unregistered path stores nothing'
);
...
assert.equal(
  (await complete('dan', ev1)).error,
  'EVIDENCE_NOT_FOUND',
  'only the uploader (or Admin) confirms'
);
r = await complete('tanya', ev1);
assert.equal(r.upload_status, 'Uploaded');
assert.equal(Number(r.size_bytes), 2048, 'storage is the witness for size');
before = await counts();
assert.equal((await complete('tanya', ev1)).upload_status, 'Uploaded');
assert.equal(await counts(), before, 'confirming twice wrote');
assert.equal(
  await canStore('tanya', e.storage_path),
  false,
  'an uploaded file cannot be stored again'
);
assert.equal(
  (await begin('tanya', req1)).upload_status,
  'Uploaded',
  'a late retry sees the finished upload'
);
...
assert.equal(t.evidence_id, ev1);
assert.equal(
  (
    await one(`select contract_evidence_id from public.jobs where id=$1`, [
      jobA
    ])
...
assert.equal(e.category, 'Contract');
assert.equal(e.attached_by, people.tanya);
assert.ok(e.attached_at);
// (events of one transaction share a timestamp, so compare as a set)
assert.deepEqual((await audits(ev1)).sort(), [
  'Attach',
  'Register',
  'TaskLink',
  'Upload'
...
assert.equal(
  (
    await one(`select count(*)::int n from public.evidence where job_id=$1`, [
      jobA
    ])
  ).n,
  1
...
const auditText = JSON.stringify(
  await all(
    `select before_json, after_json, reason from public.audit_events where entity_type='Evidence'`
  )
);
assert.ok(
  !/storage_path|token|signed|http/i.test(
    auditText.replace(/Signed-Contract-final\.pdf/g, '')
  ),
  'audit leaked a path or URL'
);

// ---- cross-job: a file registered for job B is useless to job A, by path or by id ------------
r = await begin(
  'tanya',
  upload('Task', pre02B.id, { filename: 'b-contract.pdf' })
);
const evB = r.evidence_id,
  pathB = r.storage_path;
await put(pathB, 'tanya');
assert.equal((await complete('tanya', evB)).upload_status, 'Uploaded');
const pre05 = await one(
  `select app.create_task_instance($1::uuid,'PRE05','PRE05-X-'||$1::text,null,null,now(),null,null,null,null,null) id`,
  [jobA]
);
const pre05row = await one(`select * from public.tasks where id=$1`, [
  pre05.id
]);
before = await counts();
for (const payload of [
  { completion_note: 'x', evidence_path: pathB },
  { completion_note: 'x', evidence_id: evB }
]) {
  r = await cmd('tanya', {
    command_id: id(),
    command_type: 'TASK_COMPLETE',
    task_id: pre05row.id,
    expected_version: pre05row.version,
    payload
  });
...
await assert.rejects(
  db.query(`update public.tasks set evidence_id=$1 where id=$2`, [
    evB,
    pre05row.id
  ]),
...
await assert.rejects(
  db.query(`update public.evidence set job_id=$1 where id=$2`, [jobA, evB]),
  /EVIDENCE_IMMUTABLE/
);
await assert.rejects(
  db.query(`update public.evidence set storage_path='x' where id=$1`, [evB]),
  /EVIDENCE_IMMUTABLE/
);
await assert.rejects(
  db.query(`update public.evidence set task_id=$1 where id=$2`, [
    pre05row.id,
    evB
  ]),
  /R1A_CROSS_JOB_EVIDENCE/
...
await assert.rejects(
  db.query(`delete from public.evidence where id=$1`, [evB]),
  /EVIDENCE_IMMUTABLE/
);
...
assert.equal(
  await canRead('ben', `${jobA}/dropped-in.pdf`),
  false,
  'even Admin reads nothing without metadata'
);

// ---- the command itself confirms a registered file the browser never reported ------------------
r = await begin(
  'tanya',
  upload('Task', pre05row.id, { filename: 'agreement.pdf' })
);
...
ok(
  await cmd('tanya', {
    ...
    task_id: pre05row.id,
    expected_version: pre05row.version,
    payload: { completion_note: 'approved', evidence_path: r.storage_path }
  }),
  'pre05'
);
assert.equal((await evidence(ev5)).upload_status, 'Uploaded');
assert.deepEqual((await audits(ev5)).sort(), [
  'Attach',
  'Register',
  'TaskLink',
  'Upload'
...
const attach = async (path) => {
  const tk = await task(jobB, 'PRE02');
  return cmd('tanya', {
    command_id: id(),
    command_type: 'TASK_EVIDENCE_ATTACH',
    task_id: tk.id,
    expected_version: tk.version,
...
r = await begin(
  'tanya',
  upload('Task', pre02B.id, { filename: 'b-contract-v2.pdf' })
);
...
assert.equal(
  (await evidence(evB)).task_id,
  pre02B.id,
  'the replaced file is kept, still tied to its task'
);
assert.ok((await audits(evB2)).includes('TaskReplace'));

// ---- installer evidence -----------------------------------------------------------------------
const today = (await one(`select app.london_date(now())::text d`)).d;
const mkWp = async (j, trade, seq) =>
  (
    await one(
      `insert into public.work_packages (job_id, trade, required, planned_start, planned_end, status, commissioning_required, sequence)
  values ($1,$2,true,$3,$3,'Scheduled',true,$4) returning id`,
      [j, trade, today, seq]
    )
  ).id;
const roof = await mkWp(jobA, 'Roof', 1),
  elec = await mkWp(jobA, 'Electrical', 2);
await db.query(
  `insert into public.allocations (work_package_id, person_id, role, start_at, end_at) values ($1,$3,'Lead',$5,$5), ($2,$4,'Lead',$5,$5)`,
  [roof, elec, people.inst_a, people.inst_b, today]
);
const photo = (over = {}) =>
  upload('WorkPackage', roof, {
...
assert.equal(
  (await begin('inst_b', photo())).error,
  'R1C_ASSIGNMENT_DENIED',
  'not allocated to this package'
);
assert.equal((await begin('store', photo())).error, 'R1C_ASSIGNMENT_DENIED');
assert.equal(
  (await begin('inst_a', photo({ category: 'Contract' }))).error,
  'EVIDENCE_CATEGORY_INVALID'
);
assert.equal(
  (await begin('inst_a', photo({ category: undefined }))).error,
  'EVIDENCE_CATEGORY_INVALID'
);
r = await begin('inst_a', photo());
assert.ok(!r.error, JSON.stringify(r));
const evP = r.evidence_id,
  pathP = r.storage_path;
await put(pathP, 'inst_a', { mime: 'image/jpeg' });
assert.equal((await complete('inst_a', evP)).upload_status, 'Uploaded');
const iw = (type, payload, w) => ({
  command_id: id(),
  command_type: type,
  job_id: jobA,
...
r = ok(
  await cmd(
    ...
    iw(
      ...
      {
        category: 'Safety',
        description: 'Loose tiles',
        evidence: [{ storage_path: pathP }]
      },
      w
    )
  ),
...
assert.equal(e.category, 'Problem');
assert.equal(e.work_package_id, roof);
assert.equal(e.issue_id, r.issue_id);
assert.equal(
  e.filename,
  'IMG_0001.JPG',
  'file name comes from the registration, not the command'
);
w = await one(`select * from public.work_packages where id=$1`, [roof]);
r = ok(
  await cmd(
    ...
    iw(
      'IW_PROGRESS',
      { note: 'same photo', evidence: [{ storage_path: pathP }] },
      w
    )
  ),
  'reuse'
...
assert.equal(
  (await evidence(evP)).category,
  'Problem',
  'a reused file keeps its first category'
);
...
assert.equal(
  (await open('hannah', ev1)).evidence_id,
  ev1,
  'office class reads every job (job.read.all)'
);
assert.equal(
  (await open('sam', ev1)).evidence_id,
  ev1,
  'the salesperson reads their own sale'
);
assert.equal(
  (await open('sam', evB2)).error,
  'EVIDENCE_ACCESS_DENIED',
  'job.read.own stops at another person’s sale'
);
assert.equal(
  (await open('inst_a', ev1)).error,
  'EVIDENCE_ACCESS_DENIED',
  'installers never open a contract'
);
assert.equal((await open('store', ev1)).error, 'EVIDENCE_ACCESS_DENIED');
assert.equal(
  (await open('inst_a', evP)).evidence_id,
  evP,
  'allocated installer opens their package’s photo'
);
assert.equal(
  (await open('inst_b', evP)).error,
  'EVIDENCE_ACCESS_DENIED',
  'same job, other package'
);
...
await db.query(
  `update public.allocations set active=false where work_package_id=$1`,
  [roof]
);
assert.equal(
  (await open('inst_a', evP)).error,
  'EVIDENCE_ACCESS_DENIED',
  'access ends with the allocation'
);
await db.query(
  `update public.allocations set active=true where work_package_id=$1`,
  [roof]
);
await db.query(`update public.people set active=false where id=$1`, [
  people.hannah
...
assert.equal(row.added_by_name, 'Tanya');
assert.equal(row.can_open, true);
assert.equal(row.filename, `..\\..\\${jobB}/Signed Contract (final).pdf`);
assert.deepEqual(
  (await list('inst_a', { job_id: jobA })).evidence.map((x) => x.id),
  [evP],
  'installer: own package only'
);
...
assert.deepEqual(
  r.evidence.map((x) => [x.id, x.current]).sort(),
  [
    [evB, false],
    [evB2, true]
  ].sort()
...
assert.deepEqual(
  (await list('tanya', { work_package_id: roof })).evidence.map((x) => x.id),
  [evP]
);
assert.equal(
  (await list('tanya', { job_id: jobA, task_id: pre02A.id })).error,
  'R1A_INVALID_FIELDS'
);
assert.equal(
  (await list('tanya', { job_id: 'x' })).error,
  'R1A_INVALID_FIELDS'
);
...
await db.query(
  `insert into public.stock_locations (name,type,usable) values ('Main Store','Store',true), ('Quarantine','Quarantine',false), ('External','Supplier',false)`
);
assert.equal(
  (await begin('inst_a', upload('Delivery', del, { filename: 'dn.pdf' })))
    .error,
  'R1A_ROLE_DENIED'
);
assert.equal(
  (await begin('store', upload('Delivery', id(), { filename: 'dn.pdf' })))
    .error,
  'R1C_DELIVERY_NOT_FOUND'
);
r = await begin(
  'store',
  upload('Delivery', del, { filename: 'dn.pdf', category: 'Contract' })
);
...
r = ok(
  await cmd('store', {
    ...
    payload: {
      ...
      delivery_note_path: r.storage_path,
      lines: [{ order_line_id: line, quantity_good: 50 }]
    }
  }),
  'goods in'
);
...
assert.equal(e.job_id, jobA);
assert.equal(e.uploaded_by, people.store);
assert.equal(
  (await open('store', evD)).evidence_id,
  evD,
  'Store reads delivery notes'
);
assert.equal((await open('inst_a', evD)).error, 'EVIDENCE_ACCESS_DENIED');

// ---- metadata and storage cannot drift apart silently ----------------------------------------------
const consistency = async (who) => {
  await as(who);
  try {
    return (await db.query(`select public.evidence_consistency() r`)).rows[0].r;
  } catch (err) {
    return { error: err.message };
  }
};
assert.equal(
  (await consistency('tanya')).error,
  'R1A_ROLE_DENIED',
  'Admin only'
);
r = await begin(
  'tanya',
  upload('Job', jobA, { filename: 'never-arrives.pdf', category: 'Other' })
);
await db.query(
  `update public.evidence set registered_at = now() - interval '2 hours' where id=$1`,
  [r.evidence_id]
);
...
assert.deepEqual(
  c.pending_stale.map((x) => x.evidence_id),
  [r.evidence_id]
);
assert.deepEqual(
  c.file_missing.map((x) => x.evidence_id),
  [ev1]
);
...
const report = (who, evidenceId) =>
  rpc(who, 'evidence_report_missing', evidenceId, 'uuid');
assert.equal(
  (await report('inst_a', ev1)).error,
  'EVIDENCE_NOT_FOUND',
  'only someone who may read it'
);
...
for (const code of [
  ...
  assert.ok(
    d.message && !/went wrong/i.test(d.message),
