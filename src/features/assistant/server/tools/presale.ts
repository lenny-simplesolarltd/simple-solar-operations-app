...
export const getPresaleWorkflowTool: ReadTool<Record<string, never>> = {
  ...
  async execute() {
    ...
    const onSubmit =
      'Submitting records the customer, the job (with its SS- reference), the presale snapshot and the follow-up tasks in one transaction. The presale is the record of what was sold and cannot be edited afterwards. Staff submit sales from Presales > New presale; the assistant cannot submit one.';
    return {
      ok: true,
