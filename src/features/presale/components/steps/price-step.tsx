...
import { breakdownRows, depositRows, type Pricing } from '../../designer/calc';
import {
  BIRD_OPTIONS,
  DONGLE_OPTIONS,
  EV_OPTIONS,
...
export function TotalsBreakdown({ pricing }: { pricing: Pricing }) {
  return (
    <>
      {breakdownRows(pricing).map((row) => (
        <TotalsRow
          key={row.key}
          label={row.label}
          value={`${row.negative ? '−' : ''}${money(row.amount)}`}
    ...
    <section aria-label='Price'>
      ...
      >
          ...
          {p.inverterLines.map((line, i) => (
            <div className='inv-line' key={line.id}>
              <select
                ...
                onChange={(e) =>
                  update((d) =>
                    patchInverterLine(d, line.id, { modelId: e.target.value })
                  )
                }
              >
              ...
              <button
                ...
                onClick={() => update((d) => removeInverterLine(d, line.id))}
              >
                ✕
              </button>
            </div>
        ...
        <select
          ...
          onChange={(e) =>
            update((d) => patchPricing(d, { stackedBattery: e.target.value }))
          }
        >
        ...
        <div className='param-grid'>
          ...
          <label>
            ...
            <select
              ...
              onChange={(e) =>
                update((d) =>
                  patchPricing(d, { finance: e.target.value as FinanceChoice })
                )
              }
            >
      ...
      <Card
        title='Total'
        hint='Labour, delivery/waste, discount and VAT are worked out automatically and appear here in the final breakdown — head office adds any non-standard labour or delivery cost manually via Auxiliaries on the Extras card above.'
      >
        {pricing ? (
          <TotalsBreakdown pricing={pricing} />
        ) : (
          <p className='hint'>Choose a panel type on the Panels step first.</p>
        )}
      </Card>
