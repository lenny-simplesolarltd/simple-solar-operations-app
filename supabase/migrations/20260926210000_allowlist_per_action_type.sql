-- =============================================================================
-- Choosing a recipient IS the authorisation.
--
-- outbound.allowed_recipients was the last door before go-live, and for
-- merchant and scaffolder email it still is: those recipients are DERIVED from
-- job data, so a bug there can address a stranger, and the list is what stands
-- between a mistake and a supplier.
--
-- A report is not that. A person with forms.send or programme.manage typed
-- those addresses into a screen and pressed save. Requiring them to ALSO be
-- named in a settings row nobody outside this repository knows exists does not
-- make anything safer - it means somebody adds a colleague to a report, the
-- screen says Queued, and the report never arrives, with the reason visible
-- only to whoever thinks to read app.email_claim_decision. That is a worse
-- failure than the one the rail was protecting against, because it is silent.
--
-- So the rail is now per action type rather than global, and DEFAULTS TO ON: a
-- new type must opt out deliberately, in a migration, with a reason. Only
-- EmailReport opts out.
--
-- What still stands for a report: FN-23 must be Automated, email.mode must be
-- LIVE, a real sending mailbox must be configured, the recipients must be a
-- valid list on the communication, and the content must still hash to what was
-- approved. A report cannot reach anybody a permitted person did not name.
--
-- ROLLBACK:
--   begin;
--   update app.outbox_action_types set allowlist_required = true;
--   -- and restore app.email_claim_decision from 20260926200000
--   commit;
-- =============================================================================
...
comment on column app.outbox_action_types.allowlist_required is
  'Whether outbound.allowed_recipients gates this type. True for types whose recipients are derived from job data (a bug could address a stranger). False only where a permitted person names the recipient directly, which is the authorisation.';

update app.outbox_action_types
set allowlist_required = false
where action_type = 'EmailReport';
...
begin
  ...
  -- The allow-list guards the types whose recipients are DERIVED from job data
  -- - a merchant, a scaffolder - where a bug could address a stranger. It does
  -- not guard a type whose recipients a permitted person typed into a screen:
  -- there, choosing the recipient IS the authorisation, and asking for it twice
  -- only means a report silently does not arrive.
  if coalesce(v_action.allowlist_required, true) then
